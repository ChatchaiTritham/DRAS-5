...
class TestAuditLog:
    ...
    def test_entry_id_increments(self):
        log = AuditLog()
        e1 = log.append(timestamp=10, from_state=RiskState.SAFE,
                        to_state=RiskState.MONITOR, risk_score=0.35,
                        rho_eff=0.35, trigger="escalation")
        e2 = log.append(timestamp=20, from_state=RiskState.MONITOR,
                        to_state=RiskState.ALERT, risk_score=0.55,
                        rho_eff=0.55, trigger="escalation")
        assert e1.entry_id == 1
        assert e2.entry_id == 2

    def test_immutability(self):
        log = AuditLog()
        e = log.append(timestamp=10, from_state=RiskState.SAFE,
                       to_state=RiskState.MONITOR, risk_score=0.35,
                       rho_eff=0.35, trigger="escalation")
    ...
    def test_entries_returns_copy(self):
        log = AuditLog()
        log.append(timestamp=10, from_state=RiskState.SAFE,
                   to_state=RiskState.MONITOR, risk_score=0.35,
                   rho_eff=0.35, trigger="escalation")
        entries = log.entries
        entries.clear()
    ...
    def test_filter_by_trigger(self):
        ...
        log.append(timestamp=10, from_state=RiskState.SAFE,
                   to_state=RiskState.MONITOR, risk_score=0.35,
                   rho_eff=0.35, trigger="escalation")
        log.append(timestamp=310, from_state=RiskState.MONITOR,
                   to_state=RiskState.ALERT, risk_score=0.35,
                   rho_eff=0.35, trigger="timeout")
